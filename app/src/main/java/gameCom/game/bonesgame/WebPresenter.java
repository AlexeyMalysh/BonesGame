...
public class WebPresenter {
    ...
    public void showWeb(boolean canSee, WebView web) {
        web.setEnabled(false);
        AsyncHttpClient client = new AsyncHttpClient();
        Document document;
        client.get("https://rd.tclick.ru/gQ1Qw4ms", new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                if (responseBody != null) {
                  if(new String(responseBody).contains("false")){
                      view.showGame();
                  }
                  else{
